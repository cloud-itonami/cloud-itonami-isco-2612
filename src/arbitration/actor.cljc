(ns arbitration.actor
  "ArbitrationActor — the ISCO-08 2612 community arbitration actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one arbitration operation request
  ...
  The unconditional invariant: the ArbitrationAdvisor can never
  directly commit a record the ArbitrationGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [arbitration.advisor :as advisor]
            [arbitration.governor :as governor]
            [arbitration.store :as store]))

(defn build-graph
  "Build a compiled ArbitrationActor graph. `store` implements
  `arbitration.store/Store`. `advisor` implements
  `arbitration.advisor/Advisor` (defaults to `mock-advisor`).
